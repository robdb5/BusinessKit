package 
{
	...
	public class panel_4 extends MovieClip
	{
		...
		public function init()
		{
			...
			main.x = 385,main.y = 329;
			main.width = 770,main.height = 440;
			addChild(main);
			setChildIndex(main,1);

			xmlLoader.load(new URLRequest("xml/data4.xml"));
		...
		function ButtonClick(event:MouseEvent):void
		{
			...
			{
				trace("fs");
				fscommand("exec","SB10029_101014MR.exe");
			}
			if (event.currentTarget.p == "fs1")
			{
				trace("fs1");
				fscommand("exec","SB10030_101414MR.exe");

			}
			if (event.currentTarget.p == "fs2")
			{
				trace("fs2");
				fscommand("exec","SB10216.exe");

			}
			if (event.currentTarget.p == "fs3")
			{
				trace("fs3");
				fscommand("exec","SB1018.exe");

			}
			if (event.currentTarget.p == "fs4")
			{
				trace("fs3");
				fscommand("exec","Li7242.exe");

			}
			if (event.currentTarget.p == "fs5")
			{
				trace("fs4");
				fscommand("exec","di1004.exe");

			}
			if (event.currentTarget.p == "fs6")
			{
				trace("fs6");
				fscommand("exec","DI2101.exe");

			}
			if (event.currentTarget.p == "fs7")
			{
				trace("fs7");
				fscommand("exec","AS2007.exe");

			}
			if (event.currentTarget.p == "fs8")
			{
				trace("fs8");
				fscommand("exec","AS3007.exe");

			}
			if (event.currentTarget.p == "fs9")
			{
				trace("fs9");
				fscommand("exec","AN7537_FINAL.exe");

			}
			if (event.currentTarget.p == "fs10")
			{
				trace("fs10");
				fscommand("exec","SB10211_070914LR.exe");

			}
			if (event.currentTarget.p == "fs11")
			{
				trace("fs11");
				fscommand("exec","DMD4111.exe");

			}
			if (event.currentTarget.p == "fs12")
			{
				trace("fs12");
				fscommand("exec","SB1015_QuickClose_021214.exe");

			}
			
			if (event.currentTarget.p == "fs13")
			{
				trace("fs12");
				fscommand("exec","Quickclose_for_lenders.exe");
